  ...
  const getUpComingEventData = async () => {
    try {
      const res = await axios.get(
        "https://mlsa-backend-4w03.onrender.com/api/event/allEventsupcoming"
      );
  ...
  const getEventData = async () => {
    try {
      const events = await axios.get(
        "https://mlsa-backend-4w03.onrender.com/api/event/allEventspast"
      );
    ...
    <>
      ...
      {!isLoggedIn && (
        <div className="container mx-auto py-8 bg-black">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mx-auto px-4 lg:ml-28 sm:ml-10 sm:mr-8">
            {" "}
            {/* Added ml-64 for large screens */}
            {loading ? (
              <Loading />
